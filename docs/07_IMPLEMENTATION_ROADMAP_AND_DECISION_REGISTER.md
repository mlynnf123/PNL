...
| 6. Spreadsheet migration          | **Done** (2026-07-21)           | Four new tables (`import_batches`, `import_source_rows`, `import_exceptions`, `import_record_links` — see ADR-004) plus a pure parser lib (`src/lib/import/`) that reads spreadsheet cells without ever coercing text or a broken formula to zero. `createImportBatch` fingerprints the workbook (SHA-256), extracts each row's raw cells + formulas immutably (exceljs — ADR-003), normalizes money/rate/date, runs the deterministic validators (`#REF!`, `60`-as-rate, narratives in money cells, split reps, missing identity, duplicates, negative profit), and writes one exception per finding — a partial unique index on (org, file hash) rejects re-uploading a live file. `getImportPreview` shows source-vs-target per row with the exception queue; `commitImportBatch` writes each committable row as **unverified Draft** records (revenue/costs contribute nothing to the financial summary until approved), never sets a job to Closed, and links every created record with a unique idempotency key so a re-commit produces zero duplicates; `resolveImportRow` is the identity queue (supply address/payout or exclude), `rollbackImportBatch` reverses a batch only while no imported job has downstream activity (audit events preserved). `buildImportReconciliation` accounts for every source row and ties source totals to created openings (Stage-7 snapshot stored on the batch). 41 tests (21 unit for the parser, 20 integration across parse/preview/commit/resolve/rollback/reconciliation) plus a real end-to-end run of the actual 86-row workbook: 53 committed, 33 blocked, every row accounted for, source totals exact. Exit criteria "staging dry run accounts for every source row; owner signs off on exceptions/opening balances" — the accounting and exception queue are built; owner sign-off is an operational step, not code. Deliberately deferred: structured address parsing (single-line source address stored in line 1, city/state/zip blank until an owner supplies them), auto-matching rep name strings to users (only an explicit resolution assigns a seller), and importing commission/payment history (narratives are queued, never auto-posted — docs/05 S7/S8).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 7. Pilot and production hardening | **In progress** (2026-07-22)    | First hardening slice delivered: **Settings & admin**. New commands — `assignRole`/`revokeRole`/`reactivateUser` (`src/server/commands/user-roles.ts`), `createRole`/`setRolePermissions` (`roles-admin.ts`, records the old→new permission delta per docs/06 SS8), and the commission rule-set lifecycle `createCommissionRuleSet`/`addCommissionRule`/`activateCommissionRuleSet` (`commission-rule-admin.ts`): a rule set is a Draft until activated, rules can only be added to a Draft (an Active/Retired set is immutable so approved history stays reproducible), and activation closes the prior open set's effective window the day before the new one begins so windows never overlap and selection stays deterministic (docs/06 SS6) — approved jobs snapshot their own rule set, so nothing recalculates history. New read models: `listUsersWithRoles`/`listRolesWithPermissions` (`settings-directory.ts`), `listCommissionRuleSets` (`commission-rule-sets.ts`), and `getAuditLog`/`getAuditFilterOptions` (`audit-log.ts`, the read side of the append-only ledger — docs/06 SS12). New screens: `/dashboard/settings` (users, roles & permissions, commission rules) and `/dashboard/audit` (filterable timeline). 10 new integration tests (settings-admin + audit-log), all gated by `SETTINGS_MANAGEMENT`/`AUDIT_VIEWING` with authorization-denial coverage — full suite now 117 integration + 36 unit, all passing; build clean; routes verified healthy. This closes the Phase 1 "settings screens" gap and the Phase 5 "audit log browsing screen" deferral, and supports the "production roles reviewed using test accounts" release gate (docs/06 SS15). **Second slice: optimistic concurrency on the job record** (docs/06 SS10, database rule "prevent silent last-write-wins"). `jobs.row_version` was a dead column; a new `updateJob` helper (`src/lib/concurrency.ts`) is now the single path for every job mutation — it always advances `row_version` and, given an `expectedRowVersion`, rejects a stale write with `ConcurrencyConflictError` rather than clobbering. All 8 job status-transition updates route through it; `approveFinancialClose` and `reopenFinancials` accept `expectedJobRowVersion`, wired from the close page's forms so a stale submit is caught and shown as a refresh-and-retry banner. 3 new integration tests — full suite now 120 integration + 36 unit. Remaining Phase 7 gates are operational or need owner input: D-001 Charlie / D-002 rounding, backup/restore verification, monitoring stack, documents & private object storage (D-020 storage vendor), structured logging, security review, and cutover approval.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
**Resolved 2026-07-31 — owner-only per-deal commission model (D-001..D-004).** JJ Roofing Pros has no sales reps; commission is owner-only (Ian, Justin, Meranda). The org-level rule-set engine (`commission_rule_sets`/`commission_rules`) is replaced by a **per-deal split** authored on each job by the deal's creator (`jobs.deal_owner_user_id`, carried from `leads.created_by`), editable only by that creator or an owner-admin (`SETTINGS_MANAGEMENT`). Recipients must be owners. **Meranda** is the automatic universal-share recipient (`organizations.universal_share_user_id`), taking **10%** of every deal (D-004). Total commission — authored shares + Meranda's 10% — may not exceed **70%** of commissionable profit, so the company always keeps ≥ 30% (D-001, replacing the 80% Charlie figure; the standard-rep rule is removed). Amounts are exact to the cent with no residual-reallocation policy (D-002). The governing-date question (D-003) is moot: the split is authored per deal, not selected by date. Implemented in migration `0015`; see `src/server/commands/commission-splits.ts` and `commission-batch.ts`.
...
```text
ADR-003: Spreadsheet parsing library for the migration importer
Status: Accepted
Date: 2026-07-21
Decision owner: Claude Code (technical), for Phase 6 (docs/05)

Context
  Phase 6 must read the JJ Roofing "Job Profit" .xlsx and preserve each cell's
  formula and cached result (docs/05 S3 "Preserve source", S5.2). The parser
  runs server-side inside a command, never in the browser bundle.

Options considered
  1. SheetJS `xlsx` — widely used, but the maintained builds are no longer
     published to the public npm registry (only the authors' own registry),
     which complicates `npm install`, CI, and audit.
  2. `exceljs` — published to npm, actively maintained, exposes both a cell's
     formula text and its cached result (including error results such as
     `#REF!`), which is exactly what the raw-extraction stage needs.

Decision
  Use `exceljs`. Workbook extraction lives in src/lib/import/workbook.ts and is
  the only module that touches exceljs; it reduces each cell to a neutral RawCell
  ({ type, value, formula }). All normalization and validation downstream is pure
  and library-agnostic, so replacing the parser later touches one file.

Consequences and tradeoffs
  - One npm dependency; no private-registry configuration.
  - exceljs loads the whole workbook into memory — fine for an 86-row internal
    sheet, revisit only if a future import is far larger.
  - Untrusted workbook macros are never evaluated (docs/05 S5.2); only cell
    values, formulas, and cached results are read.

Affected requirements/tests/migrations
  docs/05 S3/S5. Covered by src/lib/import/parser.test.ts (pure) and the import
  command integration tests.

Rollback or supersession path
  Swap the implementation of src/lib/import/workbook.ts; the RawCell contract and
  everything downstream is unchanged.
```

```text
ADR-004: Import lineage in dedicated tables, not per-table import columns
Status: Accepted
Date: 2026-07-21
Decision owner: Claude Code (technical), for Phase 6 (docs/05)

Context
  Every imported record must trace back to its batch and source row, re-running
  an import must not duplicate records, and the raw workbook rows and their
  exceptions must be preserved (docs/05 S3 "Traceability"/"Idempotency"/"Preserve
  source"). The alternative — nullable import_batch_id/source_row_id columns on
  jobs, customers, revenue_components, cost_transactions, job_adjustments — would
  scatter migration concerns across every business table and still not hold the
  raw rows or exception queue.

Decision
  Four dedicated tables: import_batches (file fingerprint + status + Stage-7
  reconciliation snapshot), import_source_rows (immutable raw cells + normalized
  candidates + per-row status + owner resolution), import_exceptions (one row per
  validation finding, with severity and resolution state), and import_record_links
  (batch + source row + created entity + a unique idempotency key). Business
  tables stay clean; lineage is a join away, and the unique idempotency key makes
  re-committing a batch a no-op.

Consequences and tradeoffs
  - Tracing a job to its source row is a join through import_record_links rather
    than a column read — an acceptable cost for keeping the financial tables free
    of migration-only columns.
  - The raw workbook rows live in import_source_rows.raw_json (write-once by
    convention), so the source is preserved without retaining the file blob in
    the database.

Affected requirements/tests/migrations
  docs/05 S3/S5/S12. drizzle/0005_*. Covered by the import command and query
  integration tests.

Rollback or supersession path
  Superseding requires a new ADR and a migration; imported business records would
