...
Next (woven, in order): Job detail two-column + lifecycle tracker + activity timeline (from `audit_events`); action-first home dashboard; command palette + status/empty-state rollout; Leads queue upgrade; then Estimates (F) and Contracts (G) built natively in the cockpit, and the jobs production pipeline (H).
