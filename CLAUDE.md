...
- **C — Calls:** `calls` table (transcript/appointment JSONB); `src/server/commands/calls.ts` + `src/server/queries/calls.ts` (log/update/delete/convertCallToLead). The `/dashboard/calls` **page was later removed** from the frontend on request — the backend (schema, commands, queries, tests) is intact so the page can be restored. Voice-agent auto-ingestion deferred.
...
- **E — Document Templates:** reusable contract/estimate line-item templates that feed the Estimate and Contract builders. `document_templates` table + `document_template_type` enum (line items as JSONB) — migration `0009`. Commands `src/server/commands/document-templates.ts` (create / update with rowVersion guard / duplicate / delete, all `crm_management`-gated + audited; the server recomputes each line total and drops empty rows — never trusts the client's total). Queries `src/server/queries/document-templates.ts` (`listDocumentTemplates` type-filtered, `getDocumentTemplate`). Tests `src/server/commands/document-templates.integration.test.ts` (create+sanitize / filter / update-concurrency / duplicate / delete / authz). **There is no standalone Templates page** (removed on request — it only fed Estimates/Contracts). Management now lives inline via `src/components/templates/template-manager.tsx` (+ `actions.ts`): a **Templates** button on the Estimates and Contracts list pages opens a type-scoped modal (create/edit/duplicate/delete). Applying a template ("Add from template") happens in each builder — estimate templates → an estimate option; contract templates → appended contract line items (+ terms/warranty if empty).

**Nav simplification (on request):** the Reports page and Calls page were removed from the topbar; the Audit log moved under Settings (`/dashboard/settings/audit`, with an Audit-log card on the settings hub and the hub reachable by `audit_viewing` alone). The report queries + CSV export route stay (the dashboard still uses the queries). The standalone Templates page was later removed (management moved inline onto Estimates/Contracts). Topbar: Dashboard / Leads / Estimates / Contracts / Jobs / Import / Settings.

**Operational-cockpit UX direction (current):** keep the topbar + slate/teal + font, but adopt the interaction patterns of an operational cockpit (persistent shell, action-first home, status work queue, scan→preview→commit, two-column record detail, command palette, semantic status). Jobs are the primary work queue. Shipped so far: `src/lib/status.ts` (central status→tone map + `humanizeStatus`), `EmptyState`, and the **Jobs work queue** — `/dashboard/jobs` restyled to slate with a KPI row, close-status buckets with counts, a URL-driven filter bar defaulting to the **last 30 days** (with "All time"), and a **preview drawer** (`jobs-queue.tsx`, `jobs-filters.tsx`, `src/server/queries/jobs-list.ts`). P/L data is **filtered, never deleted** — the 30-day default hides historical jobs (kept in the DB).

Cockpit surfaces shipped: Jobs work queue; two-column job detail (lifecycle tracker + activity timeline via `getEntityActivity`); action-first home; command palette (⌘K, `src/app/dashboard/command-palette.tsx`) + centralized status colors + shared `EmptyState`; Leads status buckets.

- **F — Estimates (complete):** `estimates` table (`estimate_status` enum; options stored as JSONB; per-org `estimate_number`; `cover_photo_key`, `lead_id`) — migration `0010`. Pure math `src/lib/estimate-math.ts` (optionSubtotal/optionTotal/estimateTotal + sanitize) — the server recomputes `total`, never trusts the client. Commands `src/server/commands/estimates.ts` (create with generated number / update with rowVersion guard / status / `updateEstimateCover` [sets cover doc id WITHOUT bumping rowVersion so an open builder token stays valid] / delete, `crm_management`-gated + audited). Queries `src/server/queries/estimates.ts` (`listEstimates` filtered incl. `leadId`, returns `leadId`; `getEstimate` full). UI: `/dashboard/estimates` (cockpit list — status buckets, KPIs, search) + a full-screen builder (`estimate-builder.tsx`, `/new` and `/[estimateId]`) with a multi-option editor (per-line or lump pricing, discount, tax, live totals, sticky save bar), **cover-photo upload** (via the documents/storage layer, served through `/api/documents/[id]`), **printable/PDF export** (`/[estimateId]/preview`, inline-hex printable + jsPDF/html2canvas-pro lazy-loaded), **template application** (Add-from-template appends an estimate-type document template as a priced option — `templates.ts`), and **lead linkage** (create-from-lead prefill via `?leadId=`, linked-to-lead note, per-lead estimate list + New-estimate button in the lead drawer). Tests: `estimate-math.test.ts` (4) + `estimates.integration.test.ts` (6).

- **G — Contracts + e-signatures + PDF (complete):** `contracts` table (`contract_status` enum draft/sent/signed/completed; line items, payment schedule, and signatures as JSONB; per-org `contract_number`; `lead_id`/`job_id`) — migration `0011`. Pure math `src/lib/contract-math.ts` (lineTotal/contractTotal/scheduledTotal + sanitize line items & payment schedule) — the server recomputes `total`, never trusts the client. Commands `src/server/commands/contracts.ts` (create with generated number / update with rowVersion guard / status / `signContract` [records a captured signature into signaturesJson WITHOUT bumping rowVersion; a customer signature auto-advances draft/sent→signed] / `seedRevenueFromContract` [offered, not automatic: creates a job `original_contract` revenue component from the contract total and links the job — requires `financial_entry`, not just `crm_management`] / delete). Queries `src/server/queries/contracts.ts` (`listContracts` filtered incl. `leadId`, `getContract` full). E-signatures use a dependency-free canvas pad `src/components/ui/signature-pad.tsx` (pointer events → PNG dataURL) uploaded as a private document (entityType `contract`) and served via `/api/documents/[id]`. UI: `/dashboard/contracts` (cockpit list) + full-screen builder (`contract-builder.tsx`, `/new` and `/[contractId]`) with line-item and payment-schedule editors, two signature cards, and a Seed-job-revenue card (job picker, `financial_entry`-gated); `/[contractId]/preview` printable + PDF (`contract-preview.tsx`, jsPDF/html2canvas-pro). Lead linkage mirrors estimates (create-from-lead via `?leadId=`, per-lead contract list + New-contract button in the lead drawer). Topbar gains Contracts. Tests: `contract-math.test.ts` (5) + `contracts.integration.test.ts` (8, incl. signature/status/rowVersion, revenue-seeding, lead filter, and two authz-denial cases).

- **Estimate engine — EP-1 (complete; replaces the Phase-F flat estimate builder):** a JobNimbus-style, **template-driven, versioned, page-based document engine** with a tiered pricing engine, in-person signing, and a branded PDF — recreating JJ's real estimate/contract outputs (the 4 reference PDFs). Migration `0013`. Schema (`src/db/schema.ts`): reusable **layouts** (`estimate_layouts` → `estimate_layout_versions` [immutable once published] → `estimate_layout_pages`), reusable per-page content blocks (`estimate_content_templates`), and **documents** (`estimate_documents` instantiated from a published layout version → `estimate_pages` [inherited until `is_overridden`, excludable] → append-only `estimate_document_versions` frozen on send/sign, mirroring `financial_close_versions`). New permission `estimate_layout_admin`. Pure libs: `src/lib/estimate-doc-math.ts` (options→sections→line-items, price-only, server-authoritative), `src/lib/estimate-tokens.ts` (`{{group.field}}` merge tokens + resolver), `src/lib/estimate-pages.ts` (page shapes + default stacks). Commands/queries/actions: `src/server/commands/estimate-layouts.ts` (create/edit-page/reorder/publish/discard — **editing a published layout forks a draft**/duplicate/retire), `estimate-content-templates.ts`, `estimate-documents.ts` (create-from-layout/page+meta edits with rowVersion concurrency + total recompute/cover/**send+sign freeze versions**/revise/void/delete); queries `estimate-layouts.ts` + `estimate-documents.ts`. UI: `/dashboard/estimate-layouts` (layout list + page-builder, `estimate_layout_admin`), `/dashboard/estimates` (WorkQueue list), `/dashboard/estimates/new` (layout selector — pick before creating; prefilled from `?leadId`), `/dashboard/estimates/[documentId]` (page-rail builder), `/[documentId]/review` (Review & Share + in-person Sign Now → immutable version), `/[documentId]/preview` (**JJ-blue** page-aware renderer + one-canvas-per-page PDF; app chrome stays slate/teal). Shared page editors in `src/components/estimate/` (used by BOTH the layout builder and the estimate builder) with an Insert-Token picker. Seed publishes two starter layouts (Repair Estimate, Full Roof Replacement). Tests: `estimate-doc-math.test.ts` + `estimate-tokens.test.ts` (unit) + `estimate-engine.integration.test.ts` (5: create-from-layout page copy, pricing reconciliation to $2,200, send+sign double version freeze + edit-lock, revise, published-layout fork immutability, authz). Full suite green. **Deferred to later EP slices:** inspection photo/text editor + full-page custom uploads (EP-2), the `legal_document` doc-kind consolidating the Phase-G Contracts domain (EP-3), and emailed remote "Send for Signing" (EP-4, needs an email provider + tokened public route — neither exists yet). The old flat `estimates` table + `src/server/commands/estimates.ts` / `queries/estimates.ts` / `src/lib/estimate-math.ts` are now orphaned (deprecated, pending removal).

Phase H is complete: production pipeline (Kanban + phase stepper), the slate/teal restyle, and **job documents** (a Documents section on the job detail — upload/download/delete via the storage/documents layer, served through `/api/documents`). Supplements need no separate UI — the job's existing Revenue components section already adds and approves `supplement`-type revenue components.

Next: estimate-engine EP-2+ (inspection page editor, the `legal_document` doc-kind consolidating Contracts, remote signing) as desired. Custom manufacturer-collateral upload pages are dropped by decision (internal app, no external integrations).
