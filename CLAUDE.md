...
- **E — Document Templates:** reusable contract/estimate line-item templates that feed the Estimate and Contract builders. `document_templates` table + `document_template_type` enum (line items as JSONB) — migration `0009`. Commands `src/server/commands/document-templates.ts` (create / update with rowVersion guard / duplicate / delete, all `crm_management`-gated + audited; the server recomputes each line total and drops empty rows — never trusts the client's total). Queries `src/server/queries/document-templates.ts` (`listDocumentTemplates` type-filtered, `getDocumentTemplate`). UI `/dashboard/templates` (`page.tsx` + `templates-client.tsx`) — card grid, type filter, and a full template modal with an add/remove line-item editor, live totals, and contract-only terms/warranty. Tests `src/server/commands/document-templates.integration.test.ts` (create+sanitize / filter / update-concurrency / duplicate / delete / authz). **Still to build out:** wiring templates into the Estimate/Contract builders (Phases F/G), and richer line-item metadata (units, tax) as those need it.

**Nav simplification (on request):** the Reports page and Calls page were removed from the topbar; the Audit log moved under Settings (`/dashboard/settings/audit`, with an Audit-log card on the settings hub and the hub reachable by `audit_viewing` alone). The report queries + CSV export route stay (the dashboard still uses the queries). Topbar: Dashboard / Leads / Templates / Jobs / Import / Settings.
