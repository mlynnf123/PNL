...
- **F — Estimates (slice 1, builder + list; PDF/cover-photo still to build out):** `estimates` table (`estimate_status` enum; options stored as JSONB; per-org `estimate_number`) — migration `0010`. Pure math `src/lib/estimate-math.ts` (optionSubtotal/optionTotal/estimateTotal + sanitize) — the server recomputes `total`, never trusts the client. Commands `src/server/commands/estimates.ts` (create with generated number / update with rowVersion guard / status / delete, `crm_management`-gated + audited). Queries `src/server/queries/estimates.ts` (`listEstimates` filtered, `getEstimate` full). UI: `/dashboard/estimates` (cockpit list — status buckets, KPIs, search) + a full-screen builder (`estimate-builder.tsx`, `/new` and `/[estimateId]`) with a multi-option editor (per-line or lump pricing, discount, tax, live totals, sticky save bar). Tests: `estimate-math.test.ts` (4) + `estimates.integration.test.ts` (4). **Still to build out:** cover-photo upload (via the documents/storage layer), the printable/PDF export (jsPDF/html2canvas), template application, and lead linkage.

Next: finish Estimates (cover photo + PDF), then Contracts (G, e-sign + PDF) and the jobs production pipeline (H).
